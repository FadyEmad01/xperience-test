...
type GradualBlurProps = PropsWithChildren<{
  ...
  // Optional color tint over the blurred backdrop
  tintColor?: string; // e.g. '#60a5fa' or 'rgb(99 102 241)'
  tintOpacity?: number; // 0..1
  tintBlendMode?: CSSProperties['mixBlendMode']; // e.g. 'multiply' | 'screen'
...
const GradualBlur: React.FC<GradualBlurProps> = props => {
  ...
  const blurDivs = useMemo(() => {
    ...
    for (let i = 1; i <= config.divCount; i++) {
      ...
      // Base blur layer
      divs.push(<div key={`blur-${i}`} className="absolute inset-0" style={divStyle} />);

      // Optional tint layer that follows the same mask progression
      if (config.tintColor) {
        const tintStyle: CSSProperties = {
          maskImage: `linear-gradient(${direction}, ${gradient})`,
          WebkitMaskImage: `linear-gradient(${direction}, ${gradient})`,
          backgroundColor: config.tintColor,
          opacity: typeof config.tintOpacity === 'number' ? config.tintOpacity : 0.3,
          mixBlendMode: config.tintBlendMode,
          pointerEvents: 'none'
        };
        divs.push(<div key={`tint-${i}`} className="absolute inset-0" style={tintStyle} />);
      }
    }
